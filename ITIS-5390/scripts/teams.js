...
function switchChannel(channelKey) {
    ...
    const clickedChannel = Array.from(document.querySelectorAll('.channel-item span')).find(
        span => span.textContent.trim().toLowerCase() === channelKey
    );
    
    if (clickedChannel) {
        clickedChannel.parentElement.classList.add('active');
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    channelItems.forEach(item => {
        item.addEventListener('click', function(e) {
            e.preventDefault();
            const channelNameSpan = this.querySelector('span');
            if (channelNameSpan) {
                const channelName = channelNameSpan.textContent.trim().toLowerCase();
